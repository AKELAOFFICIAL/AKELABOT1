...
last_prediction_time = {}  # Track last prediction time for each user
    ...
    def process_new_data(self, new_records):
        ...
        if added > 0:
            ...
            if added >= 3 and len(self.data) >= self.ai_threshold and AI_AVAILABLE:
                threading.Thread(target=self.train_all_models, daemon=True).start()
            ...
            try:
                ...
                for user_id in list(prediction_queue.keys()):
                    if prediction_queue[user_id]['active']:
                        ...
                        last_time = last_prediction_time.get(user_id, 0)
                        if current_time - last_time >= 25:
                            # Get prediction
                            prediction = self.get_next_prediction()
                            
                            # Send prediction with NEW TEMPLATE
                            try:
...
🕒 𝗣𝗲𝗿𝗶𝗼𝗱 : {next_period}‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ 
🎯 𝗣𝗿𝗲𝗱𝗶𝗰𝘁𝗶𝗼𝗻:  {pred_number} ({pred_size})
🎨 𝗖𝗼𝗹𝗼𝘂𝗿: {pred_color}

⏳ Next update after 1 minute.......
                                """
    ...
    def check_predictions_against_results(self):
        ...
        try:
            ...
            for prediction in self.history[:20]:
                ...
                if predicted_period == actual_period and not prediction.get('verified'):
                    ...
                    if is_win:
                        self.send_win_notification(prediction, win_type)
                        win_detected = True
            
            # Save data
            self.save_data()
            
            # If win detected, trigger immediate next prediction
            if win_detected:
                self.trigger_immediate_predictions()
            
        except Exception as e:
            pass
    
    def send_win_notification(self, prediction, win_type):
        """Send win notification only"""
        try:
            predicted_period = prediction.get('next_period')
            predicted_number = prediction.get('predicted_number')
...
✅ Target: {target_wins} wins
✅ Auto Mode Stopped

🎯 Start again for new target!
                            """
                            bot.send_message(user_id, message, parse_mode="Markdown")
                            prediction_queue[user_id]['active'] = False
                            
                    except Exception as e:
                        print(f"Error sending sticker to {user_id}: {e}")
        except Exception as e:
            print(f"Win notification error: {e}")
    
    def trigger_immediate_predictions(self):
        """Trigger immediate next predictions"""
        try:
            for user_id in list(prediction_queue.keys()):
                if prediction_queue[user_id]['active']:
                    last_prediction_time[user_id] = 0
...
def show_main_menu(chat_id, user_id):
    ...
    btn2 = types.KeyboardButton("🔄 AUTO MODE")
    btn3 = types.KeyboardButton("🛑 STOP AUTO")
    btn4 = types.KeyboardButton("🎯 MY WINS")
    btn5 = types.KeyboardButton("🔄 RESET WINS")
...
def handle_auto_mode(message):
    ...
    if prediction_queue.get(user_id, {}).get('active', False):
        ...
        bot.send_message(
            ...
            "🔔 **NOTIFICATION:**\n"
            "• WIN: ✅ Sticker only\n"
            "• LOSS: No notification\n"
            "• Target complete: Message\n\n"
            "⏱️ **TIMING:**\n"
            "• Predictions every 25 seconds\n"
            "• Fast win detection\n\n"
            "📊 **FORMAT:** Live Prediction with new template",
            parse_mode="Markdown"
        )
...
# REMOVED: STATUS, DATA, VIP CHANNEL, HELP buttons
# But we keep the commands for admin use
...
# KEEP ADMIN COMMANDS BUT REMOVE FROM BUTTONS
...
• Prediction interval: 25 seconds
...
def handle_help_command(message):
    ...
    bot.send_message(
        ...
        "2. WIN: ✅ Sticker only (no message)\n"
        "3. LOSS: No notification\n"
        "4. Win counter tracks your success\n"
        "5. Set target wins for auto mode\n"
        "6. Target complete: Message notification\n"
        "7. Check history for past results\n\n"
        "🎯 **WIN TYPES:**\n"
        ...
        "- WIN: Sticker only\n"
        "- LOSS: No notification\n"
        "- Win detection: Immediate next prediction\n\n"
        "📊 **PREDICTION FORMAT:**\n"
        "📡 LIVE PREDICTION\n"
        "🕒 Period: xxxxxx\n"
        "🎯 Prediction: x (Size)\n"
...
def handle_loss_command(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, "ℹ️ Loss notification system disabled.", parse_mode="Markdown")
...
# --- START BOT ---
print("=" * 60)
print("🤖 Akela AI Telegram Bot - NEW TEMPLATE")
print("📡 LIVE PREDICTION Format")
print("🎯 6 Buttons Only")
print("=" * 60)
